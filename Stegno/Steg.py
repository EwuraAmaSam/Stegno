...
encryption_methods = ["Reverse", "Addition"]
...
def add_noise(file_path):
    try:
        ...
        # key = np.random.normal(0, 1, frames.shape)
...
#         # Clip the audio unto frames so it can be manipulated.
#         frames = np.array(list(clip.iter_frames()))
#         # Generate original audio by removing random noise to the frames
#         noise_removed_frames = frames - 0.2 * np.random.normal(0, 1, frames.shape)
#         return noise_removed_frames
#     except Exception as e:
#         messagebox.showerror("Error", f"Failed to add noise to audio: {e}")
#         return None
...
# Downloading the encrypted file to the computer
def download_encrypted_audio():
    ...
    if os.path.exists(audio_path):
        ...
        download_path = filedialog.asksaveasfilename(defaultextension=".wav", 
